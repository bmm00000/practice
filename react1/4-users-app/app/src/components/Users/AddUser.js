...
// the approach of using refs to interact with dom elements, specifically with input elements, has a special name: we are talking about 'uncontrolled components' if you are accesing values with a ref. therefore, our inputs would now be uncontrolled components, because their internal state (the value reflected in them) is not controlled by react (we rely on the default behaviour of the inputs, and then we just fetch it with a react feature, but we don't feed data back into the input). (when we reset the inputs, we are not using react). so we are not controlling the state of the input element with react. you can talk about controlled and uncontrolled components in the context of any component, but usually we have this scenario when we talk about input components in a form in general, because these components usually have an internal state natively by the browser, and when we want to work with those components in a react app, we want to connect our react state to that. when we use refs, we have uncontrolled components, and when we use state we have controlled components (then, the inputs internal state is controlled by react).
