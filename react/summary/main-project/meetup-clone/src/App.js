...
// However, 'Switch' checks if the url starts with the path defined, and it stops when it hits a match, therefore it will give us 'AllMeetupPages' even if the url is 'localhost:3000/favourites', for example. That's why we add the 'exact={true}' or just 'exact' (you can shorten a prop like that when you want to equal it to a truthy value). the 'exact' prop tells react router that for that particular route, it should not check whether the path begins with '/', but whether the full path matches '/'
