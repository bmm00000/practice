...
// with mocks you also replace functions, but you replace bigger parts of an api of a certain module or code, and with mocks you also implement some test specific logic in the replacement function that does something else than your original function but helps you test different scenarios.

// when you call 'mock', the auto mocking algorithm used by vitest and jest is going to look for the __mocks__ folder and use your own implementation there. only if it doesn't find one, it will authomatically replace everything with empty functions. here, since we have a file called fs.js in the __mocks__ folder, we will use it:
vi.mock('fs');
// remember, this will only impact our tests, not our production code (you only mocked the module during your tests).
// also, remember that vi.mock is hoisted to the top of the file in vitest, but not with jest! (with jest, you would have to call jest.mock() at the top of the file, before you import anything). as a result, the fs module that we import is already a mock module.
...
// when we mock 'path', we care about the replacement of the 'join' method. we don't want just an empty function that keeps track of executions. we want to create a replacement to the 'join' method that returns a storagePath, but we want to simplify this function so that the returned storagePath is just the filename that we passed to writeData (so then you will be able to check if the filename is used as storagePath when you call writeFile). in a nutshell, you want to replace the 'join' method: this is the function that you pass as a second argument to the mock method (by default (if you don't pass a second argument) vitest (and also jest) will replace all the module's functions with empty functions). in this function, you can return an object which will be used as a replacement for the module that you are mocking away (in our case, for 'path'). you have to add a 'default' key, and then you add your 'join' method (we do that because 'path' is an named import, which means that a default export was used to export it). we know that 'join' takes an infinite amount of arguments (that's why we use the rest parameters syntax), and we want to return the last argument that we passed (it will always be the filename). therefore, now 'join' just returns the filename. therefore, for our tests, storagePath will be our filename.

// as we have seen, when we use 'mock', if we don't add a second argument, the replacement will be an empty function (if it can't find any file with the same name in the '__mocks__' folder), or the function with the same name that it finds in the '__mocks__' folder (remember, you have to import it). otherwise, the replacement will be what we add as a second argument to 'mock' (so we don't need to import anything).

it('should execute the writeFile method', () => {
	const testData = 'Test';
	const testFileName = 'test.tx';
	// return expect(writeData(testData, testFileName)).resolves.toBeUndefined();
	// with the line above, the test would fail, because we would be looking for a promise that resolves to no value, but since all the functions of the module are now empty (mock), we don't get back a promise that would resolve. that's why we do the following:
	// (NOTE THAT AFTER WE USED THE IMPLEMENTATION IN __MOCKS__, IT WILL WORK, as we check in the next test!!).
	writeData(testData, testFileName);
	// expect(fs.writeFile).toBeCalled();
	expect(fs.writeFile).toBeCalledWith(testFileName, testData);
	// as a result of the line above, if you change io.js, and for example, delete filename when you call the 'join' method, the test will fail. this is the power of mocks: you can get rid of unwanted behaviours, and you can even add a behaviour that simplifies your life when you write tests.
});
// the whole point of this is: since fs is some module that we don't own, it's harder to replace with a spy (we don't receive the writeFile function as an argument that we can replace, but it's being called in a module that we don't own). with mocks we can replace functionalities that are defined in modules, no matter if we own them or not. you have to pass to the 'mock' function the path to the module to be mocked (this works with both built-in or third party modules): vitest (or jest) will find this module and replace all the functions in there with empty spy functions. in our example above, when we call writeData, it should no longer write the test.txt file, because for our tests in this file, the 'fs' module provided by node has been replaced by a mock version that has only empty functions that don't do anything.
...
// Managing Custom Mock Implementations Globally: a feature that is supported by both vitest and jest is that you can add a special folder to your project: '__mocks__' (this is a special folder name where vitest or jest will search when you call 'mock'). in that folder you can add files with the names of the modules that you want to mock away.
