...
// this works, because we changed the object which was assigned as a prototype object to the newly created objects (the companyPerson object is not being copied or re-created for every new employee object that you create, but we are always linking tao the same companyPerson object in memory, and therefore, if we manipulate the companyPerson object in memory, we manipulate one and the same object, which is linked (as a prototype) to all employees objects. that's how you can add features to all your employees objects after initially setting up the prototype).
...
// watch again video 'changing prototypes (patching)??
